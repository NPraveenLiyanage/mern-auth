...
import { useDispatch } from 'react-redux';//to dispatch userslice
...
export default function SignIn() {
  ...
  const handleChange = (e) => {
    //....use strip operator to keep the previous value of the form data
    setFormData({...formData, [e.target.id]: e.target.value});
  }

  const handleSubmit = async (e) => {
    e.preventDefault();//this prevent refreshing the page when we submit the form
    try{
      // setLoading(true);
      // setError(false);
  ...
  return (
    <div className='p-3 max-w-lg mx-auto'>
      ...
      <div className="className">
        ...
        <Link to='/sign-up'>
          <span className='text-blue-500'>Sign Uo</span>
        </Link>
      </div>
